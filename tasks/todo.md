...
- [x] 更新 v1.3.0 版本与正式更新日志
- [ ] 运行最小充分验证、生产备份、迁移、部署与真实页面验收
...
- `0021_task_lifecycle_room_operations` 已通过真实 SQLite 升级、逐级降级、再次升级及 PostgreSQL 离线 SQL；当前唯一迁移头为 0021。
...
- 开发和验收未调用 DeepSeek、Hostex 或企业微信真实接口，外部模型 token 消耗为 0，未发送真实消息；生产发布证据待完成后补充。
