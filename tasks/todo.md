...
- 稳定房间展开内容、维修/可入住色调：当前无稳定房间，也无维修或可入住状态的房间。

### 数据与运营

- **403 条历史人工跟进提醒未标记已处理**。功能已上线（关注页可勾选并批量标记），
  但一次性改 400 多条生产记录属于用户操作，未代按。
- **历史数据只读盘点未做**。代码审查报告要求盘点：缺照片但保留附件记录的任务、
  改期旧任务、疑似删除后重建的任务、`SENT` 但收到失败回执的部件、`FAILED` job
  对应的 `PENDING` 凭证。需要独立方案与授权，不混入代码修复。
- **多次部署均未做生产备份**。`.stage/remote.sh` 没有备份步骤，v1.4.0 那次的
  `/opt/yumi-backups/` 是人手做的。v1.13.0 起的部署包含真实迁移，性质与之前不同。
