...
- 外部连接测试：DeepSeek OpenAI/Anthropic 两项通过，百居易 properties 只读探针通过，企业微信回调 AES/签名本地自检通过。企业微信 KF Secret 与 Agent Secret 均返回稳定错误 `60020`，根因是固定公网 IP 尚未加入两处企业可信 IP；补拉心跳过期后整体健康会再次降级。
- 剩余人工动作：在企业微信“微信客服”API 配置和自建应用 Agent 配置中都加入 `117.72.14.15`。完成后重跑连接测试、企业微信真实消息收发和管理员登录后的配置失败保旧值/成功热切换/回滚/AI 调试无副作用验收。
