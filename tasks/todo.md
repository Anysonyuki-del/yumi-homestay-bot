# 当前任务：取消管理员新密码的 12 位最低限制

- [x] 定位浏览器表单和服务端的 12 位校验边界
- [x] 取消管理员新密码最小长度，保留非空白与 128 字符上限
- [x] 同步普通改密页、首次改密页和认证测试
- [x] 将补丁版本提升到 `v1.0.2`
- [x] 完成定向、全量和静态验证
- [x] 提交、打标签并部署云端
- [x] 验证公网版本、健康状态和运行日志
...
- 根因：普通改密页和首次改密页均声明 `minlength="12"`，服务端 `validate_new_admin_password()` 同时使用 12 位常量拦截，因此只改页面无法生效。
- 修复：仅取消管理员新密码最低长度；仍拒绝空值与全空白，保留 128 字符输入安全上限，其他密码和密钥规则未改。
- 验证：定向 `55 passed`；禁真实契约全量 `1118 passed, 15 skipped`；Ruff、mypy（108 个源码文件）、compileall、pip check、JavaScript 语法和 diff check 全部通过。
- 构建：`homestay_bot-1.0.2-py3-none-any.whl` 构建成功，包元数据版本为 `1.0.2`。
- 发布：功能提交 `31873c8` 和 `v1.0.2` 标签已推送并部署；部署前备份位于 `/opt/yumi-backups/v1.0.2-20260824T192406Z`，PostgreSQL 自定义格式备份已通过目录校验。
- 云端验收：公网 `/health` 返回 `ok`，OpenAPI 与容器内版本均为 `1.0.2`；容器内一个字符的新密码通过校验，两套密码模板均不存在 12 位限制。
- 运行状态：Alembic 为 `0018_stay_checkout_observation (head)`，PostgreSQL healthy，API 重启次数和近 10 分钟错误计数均为 0；既有三个未跟踪环境备份文件保持不变。

# 历史任务：修复云端后台静态资源缓存

- [x] 复现真实 Chrome 中新版 HTML 搭配旧版 CSS 的混合状态
...
- [x] 第一段：核对当前版本来源、页面展示、Git 标签和已有经验资料
- [x] 第二段：用户要求直接进入编码，按 `v1.0.0`、后台展示、长期手册和敏感信息排除边界执行
- [x] 第三段：采用 `pyproject.toml` 单一来源、安装包元数据读取和同号标签策略
- [x] HARD-GATE：用户明确要求直接进入编码阶段
- [x] 建立单一应用版本来源并补自动化测试
...
- 应用发布基线设为 `v1.0.0`；`pyproject.toml` 是唯一正式版本源，`version.py` 统一提供运行时版本和界面标签，源码未安装时明确显示 `development`。
- FastAPI 元数据、系统诊断和后台侧栏复用同一版本读取入口；应用发布、Alembic 迁移、运行配置 revision 与 Git 提交在 README 和手册中分开说明。
- 长期手册已归并企业微信安全改写、三秒消息合并、CRM 自动入住备注、后台响应式改版、微信服务号入口边界和 Docker 云端部署经验，并补 Evidence → Finding → Path 追溯表。
...
- [x] 第一段：审查现有前端与三张参考页面，确认现状分析
- [x] 第二段：确认页面范围、信息架构与功能点
- [x] 第三段：确认视觉系统、响应式策略、风险与技术决策
- [x] HARD-GATE：用户确认最终 Spec 后才允许修改代码
- [x] 按确认后的精确文件计划实施
- [x] 完成自动化、可访问性、响应式和真实页面视觉验收
- [x] 经用户确认后同步 Git 并部署云端

## Review

- 后台改为浅色、176px 分组侧栏、56px 顶栏和单一页面标题；总览使用 6 项紧凑指标及今日运营工作区。
- 客户、房源、任务和审批统一为桌面数据表与移动卡片；知识库新增表单默认折叠，审计表在移动端安全横向滚动。
- 表单提交增加“正在处理…”反馈、`aria-busy` 和重复提交保护；保留既有确认、脏表单、抽屉焦点和权限边界。
- 定向验证 111 项通过；禁用真实外部契约的全量验证 1113 项通过、15 项按预期跳过；Ruff、mypy、compileall、pip check、JavaScript 语法和 diff check 全部通过。
- 真实浏览器验证 375、768、1024、1440 断点无横向溢出；移动抽屉焦点进入、Escape 关闭与焦点恢复通过；浏览器控制台无警告和错误。
- 功能提交 `93bd8ac` 已推送至 GitHub `origin/main`，云端仓库已 fast-forward 到同一提交。
- 部署前备份位于 `/opt/yumi-backups/admin-ui-20260823T211912Z`；PostgreSQL 自定义格式备份已通过目录校验，`.env` 与私有上传目录按存在情况备份并收紧权限。
- API 镜像重建和容器替换成功，PostgreSQL 保持 healthy；Alembic 当前版本与唯一 head 均为 `0018_stay_checkout_observation`。
- 云端本机及公网 `/health` 均返回 `ok`；登录页返回 200，未登录后台返回 401，新版 CSS/JS 均返回 200 并命中 `176px` 侧栏与“正在处理…”标记。
- 部署后 API 重启次数为 0，近 10 分钟日志错误关键词计数为 0；云端 tracked worktree 干净，既有 3 个未跟踪环境备份文件保持不变。
...
- [ ] 真实验收：在企业微信分别询问“明天天气如何”、普通服务请求和一条高危需人工问题，核对武汉默认、亲和管家口吻、三秒合并、单次回复、高危中立模板与值班管家收尾。
  - [x] 天气消息链路：北京时间 00:33:36 入站，00:34:02 单次回复发送完成，无任务重试或 `msg_send_fail`。
  - [ ] 天气正文质量：已正确换算武汉 2026-08-22，但“带把晴雨伞”后又追加“出门记得带伞”，未通过“最多一条实用提醒”验收。
  - [x] 普通服务请求真实验收：北京时间 00:55:39 收到“请帮我送两瓶矿泉水”，只发送一条统一安抚；最终相同正文被摘要去重，员工通知完成，并创建 `SUPPLIES / PENDING_CONFIRMATION` 待确认任务。
  - [x] 高危需人工问题真实验收：北京时间 00:57:50 收到“【测试】房间有浓烟”，约 1 秒内直接发送撤离与 119 指令，再使用中立确认和值班管家收尾；客人和员工发送任务均一次完成。
...
- 天气真实验收发现：`guest_reply_policy.py::_warm_weather_reply()` 只检查连续文本“带伞”，未识别“带把晴雨伞”等同义表达，因此模型已有雨具提醒时仍追加固定提醒；本地用生产正文模式稳定复现。该问题待单独修复后复验，当前不影响单次发送与日期换算结论。
- 普通服务真实验收通过：客人侧正文为“我已收到您的诉求。我会立即联系管家来处理，请您稍等。”；快速安抚发送任务完成后，最终任务第二次领取仅用于等待安抚状态，并因最终安全正文摘要相同而未重复发送。内部生成任务准确包含“两瓶矿泉水”，状态为待确认，未向客人承诺送达结果或时间。
- 高危真实验收通过：浓烟测试绕过普通三秒静默与模型链路，客人侧只发送一条固定安全回复，安全动作位于最前，正文无道歉、无责任判断、无到场或完成承诺；`conversation_handoff` 记录为 `emergency:fire`，值班员工通知完成。随后“我要退款”也正确进入 `complaint:refund`，只发送中立确认并完成内部复核卡片任务。
## 排查 Invalid prompt 拦截

- [x] 定位报错产生时间、调用链与原始错误记录
- [x] 核对模型供应商、请求参数和提示词拼装逻辑
- [x] 对照近期变更与同类正常请求，确认触发条件
- [x] 形成根因结论与验证证据（本轮不改业务代码）

### Review

- 报错发生在 Codex 旧任务调用 `gpt-5.6-sol` 之前，连续 6 次均无模型输出，不在民宿机器人运行链路内。
- 用户输入“检查输出”“我发送了消息，验证一下”本身均为普通文本；同一旧任务累积上下文约 20.35 万输入 token，并包含火灾、燃气、暴力、医疗急症等安全测试上下文，最符合上下文级误判。
- 自动压缩后仍失败，切换权限配置也未改变结果；新任务能够正常处理，进一步排除权限、项目代码和用户短句本身。
- 生产民宿机器人使用 DeepSeek `deepseek-v4-flash`，代码和数据库均未出现该 OpenAI 报错；本轮未修改业务代码。

## 天气回复纯文本与内部证据隔离（2026-08-21）

- [x] 复核真实企业微信天气正文并定位三项缺陷
- [x] 追踪雨具提醒、Markdown 与查询证据的生成链路
- [x] 确认修复范围：联网时效依据继续保留，但统一转换为自然的民宿管家收尾
- [x] 提出方案并分段确认轻量 Spec
- [x] 用户确认完整 Spec 并明确开始实现
- [ ] 重新执行本地回归、全量检查与企业微信真实天气验收（本地已完成，待部署后企业微信实测）

### 已确认轻量 Spec

- 联网检索继续保存完整查询日期、来源标题和网址用于后台证据校验；客人侧只展示自然日期和最多两个可读的主要来源名称，不展示网址、域名或“查询日期：”“参考来源：”等技术标签。
- 中文时效收尾采用民宿管家自然表达，例如“这是我今天（8月21日）帮您查到的最新预报，主要参考了武汉市气象台等公开信息。天气可能临时变化，出门前可以再看一眼实时情况。”；英文使用等义自然表达。
- 天气、活动、门票/开放时间使用各自自然收尾；搜索证据不可靠或没有可读来源名称时继续走既有联网失败路径，不用模型常识冒充实时结果。
- 客人可见回复统一为纯文本：移除明确的 `**...**`、`__...__` 强调标记，把 Markdown 列表符转换为 `•`；不误删正文中合法的星号、下划线或普通标点。
- 模型只精炼搜索正文，日期和来源收尾由本地确定性逻辑重新拼接，避免模型删除、改写或重新暴露技术标签；不增加模型调用。
- 雨具提醒按语义去重，识别“带伞、带把伞、雨伞、晴雨伞、备伞、携带伞”等中文表达和英文 `umbrella`；同一回复最多保留一条雨具建议。
- 格式化和最终客人回复策略重复执行必须幂等；不修改日期、地点、温度、降雨概率、票价、开放时间等搜索事实。

### 精确实施计划

- [x] RED 1：修改 `tests/unit/test_tourism.py`，用真实缺陷正文验证 Markdown 清理、自然日期、最多两个来源、无技术标签/网址，以及天气、活动、门票和中英文收尾。
- [x] RED 2：修改 `tests/unit/test_guest_reply_policy.py`，验证纯文本转换、雨具语义去重和重复执行幂等。
- [x] RED 3：修改 `tests/unit/test_deepseek_client.py` 与 `tests/unit/test_deepseek_tourism.py`，验证模型只精炼正文、本地证据收尾原样重接、无可读来源继续失败。
- [x] GREEN 1：修改 `src/homestay_bot/integrations/tourism.py::format_tourism_reply()`，增加正文/自然证据收尾拆分与拼接，并限制两个可读来源。
- [x] GREEN 2：修改 `src/homestay_bot/integrations/deepseek_tourism.py::DeepSeekTourismSearcher.search()`，传入语言和时效类别，并保持现有搜索、日期与证据真实性校验。
...
- RED 证据：首轮真实缺陷用例得到 11 个预期失败；后续独立审查发现的行内旧标签、单/三重强调、雨衣语义、英文票务分类和裸 URL 后事实保留均先用独立失败用例复现，再完成最小修复。
- 最终定向验证：旅游格式化、DeepSeek 旅游/客服、客人回复策略和会话出口共 `217 passed`。
- 最终全量验证：`978 passed, 15 skipped`；跳过项均为未显式开启或缺少测试回调凭据的真实 DeepSeek、百居易与企业微信契约。
...
## 天气正文安全投递与实时旅游事实过滤（2026-08-21）

- [x] 确认客人实际只收到“正在核实”安全兜底，天气正文未送达
- [x] 核对消息元数据：天气正文 ID 49 被 `wecom_async_13` 拒绝，兜底 ID 50 已受理
- [x] 定位实时旅游分支在 `DeepSeekGuestAssistant.respond()` 中提前返回，绕过 `_validate_decision()` 的民宿专属事实过滤
- [x] 确认原天气正文夹带未经审核的“我们民宿有伞可借用”
- [x] 确认总体原则：正常回复保持完整；只有被 `fail_type=13` 拒绝后才重新组织语言，不把所有简单问题预先降级为安全短答案
- [x] 确认被拦截后允许额外调用一次 DeepSeek 改写：不重新联网、不新增事实、最多重试一次；改写或二次投递仍失败才使用分类短兜底并通知员工
- [x] 分段确认总体链路，并形成完整轻量 Spec
- [x] 用户审核完整书面 Spec 后开始修改业务代码
- [x] 按 TDD 修复、全量验证、独立复审、推送部署并重新完成企业微信天气验收

### 实施计划

- [x] RED：补充实时旅游单段正文夹带民宿自述的回归测试，证明只删除无依据句子并保留天气事实与自然证据收尾
- [x] GREEN：把民宿专属事实过滤改为逐句处理，并接入实时旅游首次回复出口
- [x] RED：补充 `fail_type=13` 只登记一次改写任务、任务载荷不含正文、非 13 仍沿用原重试的集成测试
- [x] RED：补充改写保留关键数字、拒绝新增链接/技术标签/新数字，以及模型异常时确定性兜底的单元测试
- [x] GREEN：实现 DeepSeek 单次无工具改写、本地事实校验和确定性分类兜底
- [x] GREEN：实现从数据库读取失败答案与对应问题的改写 job handler，登记一次带关联编号的客人投递
...
- [x] ACCEPT：由企业微信真实天气消息完成最终验收

### 当前验证结论

- 格式层修复已在生成正文中生效：无 Markdown、无原始查询字段、无网址，雨具提醒只有一次。
- 企业微信异步安全限制导致生成正文未送达；平台拒绝原因只提供 `fail_type=13`，不能据此断言某个具体词句是唯一触发原因。
- 现有安全重试在 `src/homestay_bot/application.py::_handle_guest_delivery_failure()` 中固定改发通用“正在核实”，能避免原文重复失败，但丢失全部已验证天气事实。
- 实时旅游路径在 `src/homestay_bot/integrations/deepseek_client.py::DeepSeekGuestAssistant.respond()` 中完成搜索和精炼后直接返回，没有经过普通回答使用的 `_validate_decision()` / `_remove_property_promotion()` 边界。

### 完整轻量 Spec

#### 正常回复与专属事实过滤

- 正常回复继续按现有链路直接发送，不增加模型调用或等待时间；只有企业微信明确返回 `fail_type=13` 后才启动改写恢复。
- 实时天气、票务、活动、路线等旅游回复在首次发送前必须经过确定性的民宿专属事实过滤；未经审核知识支持的“本民宿可借伞、提供接送、拥有某设施”等句子必须删除。
- 专属事实过滤只处理搜索正文，不删除已验证的天气、日期、价格、开放时间、路线或本地自然证据收尾；若过滤后正文为空，按现有实时查询失败路径处理，不能只发送来源收尾。
- 普通回答与实时旅游共用同一套专属事实识别规则，避免两条出口以后再次漂移；对单段长文本按句处理，不能因为一句含“我们民宿”而删除整段其他有效事实。

#### `fail_type=13` 异步改写恢复

- `src/homestay_bot/application.py::_handle_guest_delivery_failure()` 收到第一次 `fail_type=13` 时，不原文重发，也不立即发送统一“正在核实”；改为登记独立后台改写任务，失败回调快速提交。
- 任务只保存失败机器人消息编号及稳定去重键，不复制客人正文或原答案到任务载荷；worker 从数据库读取失败答案及同会话对应问题。
- 新增 DeepSeek 改写边界，最多调用一次，不启用联网或工具。输入只包含已脱敏的客人问题和已验证原答案；输出为严格结构化 `reply_text`。
- 改写必须保留原答案中的日期、地点、温度、降雨、价格、开放时间、路线、距离和必要安全提醒；不得新增事实、民宿设施、承诺、网址、来源标签或内部流程。
- 改写结果继续经过纯文本清理、承诺过滤、民宿专属事实过滤与 1500 字硬上限，再使用 `retry_of_message_id` 投递一次；不得覆盖原失败消息记录。
- 相同失败回执、worker 重试或进程重启只能产生一个改写任务和一次改写后客人投递；原消息、改写消息和员工通知保持可审计关联。

#### 最终失败与确定性兜底

- DeepSeek 改写调用异常、空白、无效 JSON、引入新数字/链接/技术标签或丢失关键事实时，不发送该改写结果。
- 改写不可用时，才根据已识别的问题类型生成确定性简短兜底：天气保留日期/地点/天气/温度/降雨和一条提醒；票务、开放时间、路线、活动保留原答案中可确定的核心结论。无法可靠抽取时使用现有“正在核实”并通知员工。
- 改写后消息若再次收到任何异步失败，不再自动改写或重发；登记一次脱敏员工跟进通知，避免循环发送。
- `fail_type` 非 13 继续沿用现有可安全重试或人工跟进规则，本次不扩大范围。

#### 验收标准

- 实时天气首次正文不含未经审核的民宿借伞等专属事实；正常未被拦截时仍只调用现有模型链路并发送一次完整回答。
- 模拟首次 `fail_type=13`：只创建一个改写任务；改写保留原天气事实、删除来源/宣传/网址并只发送一次，不出现统一“正在核实”抢先回复。
- 重复失败回执、任务重试和进程恢复不重复调用改写、不重复发送；第二次失败只通知员工。
- 覆盖中文/英文、天气/票务/路线、模型改写失败、关键事实丢失、链接重现、无可靠分类兜底和数据库异常隔离。
- 通过定向测试、禁 live 全量 pytest、Ruff、mypy、compileall、pip check、diff check、独立代码审查、云端备份部署和新的企业微信天气实测后才标记完成。

### Review（部署前）

- 独立复审最终结论：Critical 0、Important 0，Ready。
...
- 真实客人消息 ID 51 已触发完整天气回复 ID 52；原回复被企业微信 `fail_type=13` 拒绝。
- 改写任务 ID 362 只执行一次并进入确定性事实兜底，但二次消息 ID 53 仍被 `fail_type=13` 拒绝；系统按设计停止重试并登记员工通知，客人仍未拿到天气答案。
- 根因回放确认：统一回复策略压平了自然证据收尾分隔；天气开场的标点差异造成重复前缀；本地兜底未清除全部列表符和来源表达。
- 补丁改为完整模板识别自然 footer，按明确归因结构移除中英文来源且保留景点/天气/票务事实，统一清除列表符与重复开场，并在最终标签添加后限制为 320 字。
...
- 补丁部署后真实客人消息 ID 54 触发原天气回复 ID 55；平台返回 `wecom_async_13` 后只创建一个改写任务 ID 369，并只投递一次改写回复 ID 56。
- 改写回复 ID 56 获得企业微信 `accepted` 回执，未出现二次失败或重复改写；客人端确认实际收到该回复。
- 客人最终收到的正文保留 8 月 22 日、武汉、多云/局地阵雨或雷阵雨、27～36℃及防暑雨具提醒，不含来源标签、网址、列表符、重复开场或未经确认的民宿事实；本任务 ACCEPT 完成。
