...
- [x] **F-08** 凭证安全门默认日期改用 `wuhan_today`
- [x] **F-06** 订单改期后作废无执行证据的旧保洁任务（新增 `superseded_turnover` 候选条件）
- [x] **F-02** 房间就绪后按当日入住重新选订单；0 个静默、≥2 个转人工
- [x] **F-03** 凭证异步失败回写投递状态、暂停同组未发送部件并建人工任务
- [x] **F-09** stale job 清空载荷前完成凭证状态与人工任务补偿
- [x] **F-05** 对账改为入住日与退房日两次独立查询后合并；心跳原本就只在成功后刷新

### C. 排队 final 的时效校验

- [x] **F-04** 实际出站前复用 `has_newer_conversation_activity` 判定过时
