...
- [x] 新增客诉状态、脱敏分析字段、来源消息幂等和版本控制迁移。
- [x] 本地规则识别客诉，切换人工模式并发送固定安抚；不进入普通模型回复链路。
- [x] 接入 DeepSeek 结构化客诉分析，禁止责任结论、金额承诺和链接。
- [x] 注册客诉后台生成任务，生成分析后通知员工进入后台复核。
- [x] 新增员工客诉编辑页，支持保存、发送、退回重分析、关闭、CSRF 和版本冲突。
- [x] 接入应用装配、worker 重试和员工端路由。
- [x] 企业微信卡片通知替换为卡片按钮入口；真实端到端验收待本机企业微信环境执行。
- [ ] 修复延迟任务审批幂等、名称缓存和真实房间号映射。
- [x] 修复 outbox 回复在实际送达前污染机器人上下文的问题，并清理已完成延迟任务正文。

### 客诉 Review

- 定向客诉测试：44 passed。
- 全量自动化测试：423 passed、15 skipped。
- 卡片通知回归后全量自动化测试：425 passed、15 skipped。
- Ruff：全仓通过；相关 mypy：5 个源文件通过。
- 迁移在线循环因本机 PostgreSQL 未启动未执行；离线生成被既有 `0003_customer_crm` 的数据回填逻辑阻断，未把该失败归因于 `0008`。
