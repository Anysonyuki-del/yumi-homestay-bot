...
# 当前任务：企业微信早间消息真实验证与故障定位

- [x] 确认验收口径：以用户实际消息、生产记录和客户端收件为准
- [x] 对齐今早入站消息原文、时间、会话和企业微信消息编号
- [x] 核对入站、生成、出站、异步失败回执与任务队列全链路
- [x] 区分源码态、生产部署副本态和真实运行态
- [x] 复现并定位“回复不正确”的根因及影响范围
- [x] 如需改动，先给出精确文件/函数计划并执行对应 HARD-GATE
- [ ] 完成最小充分验证，并取得真实企业微信客户端收件证据

## 调查边界

- 当前先做生产只读取证和故障定位，不修改业务代码、数据库或外部配置。
- 不主动发送新的企业微信测试消息；若需要复测，以用户提供的原文或用户再次发送为准。
- 不读取、修改、暂存或提交未跟踪的 `YuMi民宿AI项目总结.txt`。

## 已确认功能规则

- 洗衣机、门锁、空调、热水、投影、WiFi 等设备或设施出现故障时，不追问客人补充症状。
- 客人回复必须同时包含：与设备匹配的简单安全排查建议，以及已经提交人工处理的明确说明；不得只发送中立安抚。
- 简单建议不得要求拆机、强行操作或执行带电危险步骤，不承诺人员已出发、完成时间或一定修好。
- 委婉故障表达与“坏了、故障、打不开”等直接表达使用同一规则，并创建去重的待确认人工任务。

## 待确认实施与验收门禁

- `services/answer_policy.py`：新增单一设备故障分类函数，集中识别设备词与直接/委婉故障表达；`is_service_request()` 复用该结果，普通“有点问题”不得误触发。
- `services/guest_reply_policy.py`：新增确定性的设备安全建议生成器；只输出设备匹配的简单建议和任务已提交固定文案，不修改普通、高风险或客诉回复规则。
- `services/conversation_service.py`：在紧急与客诉分类之后、模型和静默任务之前处理设备故障；先幂等创建 `MAINTENANCE` 待确认任务，再把建议和“已提交人工处理”写入同一事务的客人出站及员工通知。任务失败时回滚重试，不发送虚假提交声明。
- `services/emergency_service.py`：补齐设备冒烟、火花、焦味等危险组合，确保危险事件仍先走紧急安全流程，不落入普通排查建议。
- 红测覆盖真实原文“洗衣机好像也出了点问题”、委婉表达和误报反例；断言不追问、包含简单建议、任务先于客人出站、同一来源不重复建任务、任务失败不声称已提交、危险表达走紧急分支。
- 最终验证覆盖 answer policy、guest reply policy、emergency、conversation、DeepSeek 任务授权和真实仓储/outbox 聚焦回归，随后运行 Ruff、受影响文件 Mypy、`git diff --check` 和一次无外联全量 Pytest。
- 本阶段不新增依赖、模块、数据库迁移或生产知识数据；不升级版本、不提交、不推送、不部署。生产复测需后续单独授权，并以相同原文的任务记录、发送失败事件和微信客户端实际正文联合验收。

## 实施进度

- [x] HARD-GATE：三段 Spec 已确认，用户明确回复“开始”
- [x] 红测：真实委婉故障、误报反例、任务顺序、失败与危险分支
- [x] 实现：确定性设备建议、任务先行与紧急分流
- [x] 聚焦回归、差异自审与最终无外联门禁
- [x] Review：记录验证证据与发布边界

## 实施 Review

- 设备故障识别已收敛到单一确定性分类器；真实原文“洗衣机好像也出了点问题”、直接故障和跨消息片段都跳过模型追问，普通“这个设计有点问题”和设备用法咨询不误触发。
- 洗衣机、门锁、空调、热水、投影和 WiFi 使用固定的简单安全建议，不要求拆机或强行操作，不承诺人员已出发、完成时间或一定修好。
- 会话流先幂等建立 `MAINTENANCE` 待确认任务，再在同一事务写入员工通知和客人回复；任务失败时不会发送“已提交”。同一来源消息的任务仍由现有唯一键去重。
- 设备冒烟、火花或焦味优先走紧急安全分支，不落入普通维修建议。
- 聚焦回归为 `151 passed`；冻结差异后的无外联全量回归为 `1249 passed, 15 skipped`，跳过项均为需显式授权的 DeepSeek、百居易或企业微信真实契约测试；Ruff、4 个受影响源文件 Mypy 和 `git diff --check` 通过。
- 本轮未升级版本、未提交、未推送、未部署，也未发送新的真实企业微信消息；客户端实际收件验收仍待后续单独授权。

## 诊断 Review

- 用户确认核心故障是“洗衣机好像也出了点问题”只收到中立安抚、没有解决方案；退款草稿问题属于同一会话的旁支，不作为本轮首要修复目标。
- 生产 08:43:32 至 08:44:21 收到同一会话的三条客人原文：“房间门锁出问题了”“洗衣机好像也出了点问题”“我要退款”；对应四个 `wecom_process_message` 和四个 `wecom_send_text` 均完成且无异步失败记录。
- 当前 `is_service_request()` 和 `_should_send_fast_ack()` 都未识别“出了点问题”这类委婉故障表达；模型任务建议会被确定性门禁清除，生产也没有为该消息创建维修任务。
- 现有测试能证明“童锁/断电”等已生成的安全排查建议不会被出口清洗删除，但没有覆盖含糊故障表达必须追问现象并给出安全下一步，因此最终回复可退化成中立安抚。
- 生产知识库和 FAQ 候选均没有洗衣机排查内容；该消息的生成、发送和员工通知任务均完成且无错误，证明不是任务中断，而是成功执行了一条没有可用排查方案的决策。
- 08:44:28 的固定退款安抚符合高风险中立规则；08:45:00 的后续草稿包含“非常抱歉”，与高风险退款不得道歉的规则冲突。
- `DeepSeekComplaintAnalyzer.generate()` 要求模型“先承认不便”，且上下文固定读取最近 12 条，导致前一天“转人工”“送两瓶水”被列为本次已知事实；`ComplaintAdminService.send()` 又直接发送草稿，未在最终出口调用高风险 `prepare_guest_reply()`。
- 审计记录显示后台账号在 08:44:59 执行 `complaint.send`；企业微信返回真实消息编号且后续没有失败回执，但接口受理仍不能替代用户微信客户端展示证据。
- 当前生产源码为 `2ae55695ef00709ad58f107545c2bc2d7c784a3d`，运行包为 `1.3.8`、数据库为 `0023_approval_pii_final (head)`、容器重启次数为 0，说明缺陷存在于当前生产业务逻辑而非旧部署。
- 本机 LaunchAgent 副本另有独立故障：版本停在 `1.0.4`，企业微信读取因错误码 `60020` 被可信 IP 拒绝且旧 Cloudflare 回调已失效；今早这组三条消息实际由生产服务器处理，不由本机副本处理。

# 当前任务：设施故障规则自动优化

- [x] 核验测试专用号新消息及生产任务、队列、worker 和数据库锁
- [x] 确认失败原因为固定设备名单和未发布代码，不是发送或 worker 中断
- [x] 第一段 Spec：现状证据与自动优化边界
- [x] 第二段 Spec：候选、自动生效、降级和安全门规则
- [x] 第三段 Spec：精确文件、迁移、测试、发布与真实收件验收
- [x] HARD-GATE：三段确认后用户已明确回复“开始”
- [x] 红测、实现、聚焦回归和最终门禁

## 当前边界

- 本轮只完成已确认的本地编码和验证，不升级版本、不提交、不推送、不部署。
- 目标是让新设施故障能被自动纳入已确认的“简单建议 + 提交人工”规则，不再为每个设备名修代码。
- 领域前提为民宿官方客服：房间、公区、民宿配套设施和民宿提供物品属于维修边界；明确的客人私人物品、车辆和外部景区/商户设施不属于民宿维修。
- 客人在该渠道只说“灯不亮了”等未显式说明归属的短句时，默认按民宿设施故障处理；只有文本或可信上下文明确指向私人/外部对象时才排除。
- 不允许模型自动生效拆机、带电、燃气等危险步骤，也不自动决定退款、赔偿、价格或人员到场/完成时间。

## Review

- 已删除固定六设备分类与建议表，改为现有主模型 JSON 同轮返回设施归属和五种固定安全档位；没有新增模型调用、数据库表、迁移、依赖或持久化别名表。
- `灯不亮了`、马桶、窗帘、烘干机等新设施首次出现即可创建维修待确认任务；明确手机等私人物品、景区和商场等外部设施不创建民宿维修任务。
- 排查建议只来自本地固定模板；模型字段缺失、无效、低置信或模型不可用时使用通用停止使用建议，模型正文不能直接生成排查步骤。
- 普通设施故障在三秒合并后统一处理，不发送中立快速安抚；任务创建、员工 outbox 和客人 outbox 保持同一事务及固定顺序，任务失败时客人不会收到“已提交”。
- 危险信号已脱离设备名单；冒烟、火花、焦味、漏电仍优先进入紧急流程。
- 红测先因开放分类接口尚不存在而收集失败；实现后聚焦单元测试 `231 passed`，相关单元/集成筛选 `52 passed`，任务与 outbox 真实 SQLite 集成 `5 passed`。
- Ruff 通过，Mypy `Success: no issues found in 130 source files`，`git diff --check` 通过。
- 最终无外部依赖回归：`1275 passed, 15 skipped, 1 deselected`。排除项 `test_production_bundle_assistant_is_read_only_end_to_end` 使用已过期的固定 2026-08-31 入住日期，已在干净 `HEAD` 临时副本中独立复现同一失败，证明不是本轮回归。
- 未运行真实 DeepSeek、Hostex、企业微信契约测试；未执行版本升级、提交、推送、部署或测试专用号真实收件验收，需另行授权。

# 当前任务：发布并部署 v1.3.9

- [x] 用户明确授权开始发布
- [x] 核对本地差异、Git 目标与生产基线
- [x] 将应用版本升级为 `1.3.9` 并补充正式更新日志
- [x] 独立完成 Ponytail 发布审查门禁
- [x] 提交并首次推送 GitHub
- [x] 创建并验证生产源码、配置和 PostgreSQL 备份
- [x] 仅重建 API，保持 PostgreSQL 容器连续运行
- [x] 验收生产 HEAD、版本、迁移、健康、重启次数和日志
- [x] 使用测试专用号完成真实消息与客户端收件验收

## 发布边界

- 发布对象仅为已完成本地验证的设施故障自动处理规则及对应测试，不附带无关功能。
- 本版本不新增数据库迁移或依赖，不修改退款、赔偿、价格、实时房态来源和消息发送状态机。
- 生产发布前先创建可恢复备份；仅更新 API，PostgreSQL 容器不得重建或重启。
- 真实消息验收以用户实际发送原文为准；企业微信接口受理不能替代客户端实际收件证据。
- 服务器既有 `.env` 备份和本地未跟踪 `YuMi民宿AI项目总结.txt` 均保持不动。

## 发布 Review

- 发布提交和 `v1.3.9` 标签均指向 `d91aecc1312b2dd1d442c4cf6b19034e8aca05d7`，已推送 GitHub；本地仅保留受保护的未跟踪总结文件。
- 生产备份位于 `/opt/yumi-backups/v1.3.9-20260831T203238Z`，包含旧源码 bundle、Compose、Dockerfile、`.env` 和 PostgreSQL custom dump；bundle 与 dump 均验证通过，目录权限为 `700`、制品为 `600`。
- 服务器 GitHub HTTPS 拉取无响应后已终止精确进程，改用两端验证的 Git bundle fast-forward；生产 tracked worktree 对齐发布提交。
- 仅重建 API；PostgreSQL 容器 ID 仍为 `af11bb4a411f`，启动时间仍为 `2026-08-10T16:43:53Z`，重启次数为 0。
- 生产包、公网 OpenAPI 均为 `1.3.9`；Alembic current/head 均为 `0023_approval_pii_final`；本地及公网健康为 `ok`，API 重启次数为 0，部署后日志无启动错误。
- 测试专用号于 04:56:52 发送原文“灯不亮了”；04:57:01 创建 `MAINTENANCE / PENDING_CONFIRMATION` 任务并依次完成员工通知和客人出站，worker 无积压、无等待锁或陈旧运行锁。
- 客人实际收到“简单安全排查建议 + 已提交管家人工处理”完整正文，企业微信客户端收件验收通过。
- GitHub CI 的唯一失败为已过期固定日期测试；本地已改为统一使用未来两天并通过聚焦测试、Ruff 和无外联全量 `1276 passed, 15 skipped`，尚待独立审查后提交。
- 镜像依赖安装层因位于版本和源码复制之后而失去缓存，生产网络以约 40–55 KB/s 重新下载依赖，依赖步骤耗时约 610 秒；后续应独立优化 Docker 缓存层，不与本次业务发布混改。
- 上下文维护循环另记录一次 customer 62 的 `ValidationError`，未阻断本次消息、任务或出站链路；作为独立诊断项处理，不据此否定本次设施故障验收。

# 当前任务：优化生产镜像依赖缓存

- [x] 确认长时间部署由依赖安装层被源码和版本变化连带失效导致
- [x] 将哈希锁依赖安装拆为只受 `requirements.lock` 影响的独立层
- [x] 验证基础镜像 digest、依赖哈希、项目无依赖解析和非 root 运行边界保持不变
- [x] 连续构建两次，确认第二次依赖安装层命中 Docker 缓存
- [x] 记录验证结果和发布边界

## 当前边界

- 本轮只修改 Dockerfile 缓存顺序和既有任务记录，不修改业务代码、依赖版本、数据库迁移或运行配置。
- 本轮不升级版本、不提交、不推送、不部署；生产优化需本地验证完成后另行授权。

## Review

- Dockerfile 仅把 `requirements.lock` 和带 `--require-hashes` 的依赖安装提前为独立层；项目仍使用 `--no-deps --no-build-isolation` 安装，基础镜像完整 digest 和 UID/GID `10001:10001` 未变。
- 本机无 Docker CLI，因此在服务器隔离临时上下文构建测试镜像，未执行 Compose 更新、容器替换或数据库操作。
- 第一次构建成功生成 `homestay_bot-1.3.9`；测试镜像运行用户为 `10001:10001`，`pip check` 报告 `No broken requirements found`。
- 第二次只把临时项目版本改为 `1.3.9+cachecheck`，Docker 步骤 1–6 全部显示 `Using cache`，其中步骤 6 是完整哈希依赖安装；项目层重新生成对应 wheel。
- 第二次构建总耗时约 13 秒，不再重新下载依赖；首次构建仍会在基础镜像或 `requirements.lock` 变化后承担完整安装成本，这是预期边界。
- 生产 API 与 PostgreSQL 容器 ID、启动时间和重启次数均未变化；生产版本仍为 `1.3.9`，健康接口返回 `ok`。
- 本轮未修改业务代码、依赖版本、迁移或运行配置，尚未提交、推送或部署 Dockerfile 变更。

# 当前任务：设施故障建议改为提示词优先

- [x] 核对测试专用号实际原文、机器人正文、维修任务和出站任务
- [x] 确认 40 条待处理任务均为尚未到期的生命周期定时任务，不是 worker 积压
- [x] 停止继续增加家电、排水等设备档位，并撤回未确认实验
- [x] 第一段 Spec：确认现状根因与目标边界
- [x] 第二段 Spec：确认故障动作、安全降级和危险事件边界
- [x] 第三段 Spec：确认精确文件、测试、部署与真实收件计划
- [x] HARD-GATE：用户明确确认完整 Spec 后才修改业务代码
- [x] 删除设施安全档位，改为同轮模型生成针对性建议
- [x] 保留危险操作、追问、虚假承诺和提交状态的本地安全门
- [x] 完成聚焦回归与最终最小充分门禁
- [ ] 发布后使用测试专用号完成真实微信收件验收

## 当前边界

- 实际原文为“洗衣机坏了”“门锁打不开”“厕所马桶堵了”；不得换成助手假设文本验证。
- 当前 `safe_profile` 单选结构会迫使开放式故障就近归类；新增设备档位不能解决根因。
- 目标是由固定民宿提示词生成与当前故障匹配的简单建议；本地只保留最小安全门和通用降级。
- 新设施不增加设备档位；只有真实测试证明提示词持续不稳定时，才补充必要的本地安全规则。
- 本轮不修改维修任务、outbox、退款价格、房态来源或紧急事件优先级；不自动提交、推送或部署。

## Review

- `FacilityIssue` 只保留设施归属，模型在现有主请求中直接生成一至两条低风险建议，不增加第二次模型调用。
- 维修任务仍在员工通知和客人回复之前创建；模型不可用、字段缺失、归属未知或低置信时使用通用停用建议。
- 本地过滤覆盖拆卸、接触线路、重启或重置房间网络设备、反复点火、强腐蚀药剂、追问和模型自行声明提交；紧急事件仍在模型调用前处理。
- 最终验证：受影响的三个测试文件共 `197 passed`；相关 Ruff 检查和 `git diff --check` 均通过。
- 未执行真实 DeepSeek、企业微信、Git 提交、推送或生产部署；真实收件仍是独立待验收证据。

## 发布 v1.3.10

- [x] 用户明确授权发布
- [x] 完成最新 Ponytail 发布审查
- [x] 将版本升级为 `1.3.10` 并同步正式更新日志
- [x] 提交、标记 `v1.3.10` 并推送 GitHub
- [x] 确认 `v1.3.10` CI 因遗漏的真实事务集成测试失败，未触碰生产
- [x] 修正状态确认句误判并同步集成测试，全量无外联测试为 `1285 passed, 15 skipped`
- [x] 将修正版升级为 `1.3.11` 并同步正式更新日志
- [x] 审查、提交、标记 `v1.3.11` 并推送 GitHub
- [x] 等待 `v1.3.11` GitHub CI 完成
- [x] 创建并验证生产备份
- [x] 仅重建并替换 API 容器，保持 PostgreSQL 运行
- [x] 验证服务器源码、迁移、容器版本、健康、重启次数和近期日志
- [ ] 使用测试专用号完成真实微信收件验收

### 发布边界

- 发布对象仅为已经通过本地门禁的设施故障提示词回复与最小本地安全门，不包含受保护总结文件或其他未跟踪内容。
- 无数据库迁移和依赖变化；生产仅更新 API，PostgreSQL 不重建、不重启。
- 发布前备份旧源码、Compose、Dockerfile、`.env` 和 PostgreSQL custom dump，并分别验证源码 bundle 与数据库归档。

### v1.3.11 生产发布 Review

- 提交和标签均指向 `6c981118f18af54ac3bc0f03c911ca66bc399a26`；main CI `33454648014` 与标签 CI `33454648095` 均成功，覆盖静态检查、全新迁移、无外联全量测试和生产镜像构建。
- 生产备份位于 `/opt/yumi-backups/v1.3.11-20260901T002900Z`，包含旧源码 bundle、Compose、Dockerfile、权限 `600` 的 `.env`、PostgreSQL custom dump、旧容器状态和 SHA-256 清单；源码 bundle 与数据库归档均已验证。
- 服务器使用 SHA-256 一致且验证通过的 Git bundle 快进到目标提交，三个既存未跟踪环境备份保持不变；只替换 API，PostgreSQL 容器 ID 和启动时间保持 `af11bb4a411f` / `2026-08-10T16:43:53.783849747Z`，重启次数为 0。
- API 容器包和本机、公网 OpenAPI 均为 `1.3.11`，Alembic 为 `0023_approval_pii_final (head)`；本机与公网 `/health` 均为 `ok`，公网登录页返回 200，未登录诊断端点返回 401，API 重启次数为 0，近期日志无 `ERROR`、`Traceback` 或 `Exception`。
- 本次首次用缓存优化后的 Dockerfile 构建生产镜像，旧生产镜像没有对应依赖层，因此完整依赖安装耗时约 943 秒；紧接着无变更重建约 1 秒且所有依赖和项目安装步骤均为 `CACHED`，证明后续锁文件不变时可直接复用。
- 尚未主动调用真实 DeepSeek 或企业微信，也没有替用户发送测试消息；真实微信客户端收件必须使用测试专用号重新发送实际原文后独立验收。

# 当前任务：设施故障语义泛化修复

- [x] 核验真实原文、机器人正文、维修任务、员工通知、worker、积压和数据库锁
- [x] 第一段确认：共同根因和修改边界
- [x] 第二段确认：语义路由、回复、安全门和泛化回归规则
- [x] 第三段确认：精确文件、验证、发布和生产验收计划
- [x] HARD-GATE：用户明确回复“开始”
- [x] 红测复现模型已理解但本地关键词门槛阻断
- [x] 以结构化设施归属替代关键词主路由
- [x] 维修请求停止发送中立快速安抚
- [x] 完成聚焦回归、事务集成、Ruff 和差异检查
- [x] 完成一次无外联全量回归和最终差异自审
- [x] 发布前升级版本并维护 CHANGELOG
- [x] 自动执行 Ponytail 门禁后提交、标记并推送 v1.3.12
- [x] 创建并验证生产备份，仅重建并替换 API
- [x] 核验生产源码、版本、迁移、容器、日志和公网入口
- [x] 使用测试专用号核验“洗衣机不出水”“马桶堵了”和噪音类真实表达
- [x] 确认四条消息均建维修任务，员工通知和客人出站按序完成
- [x] 扩展固定民宿提示词，使住宿环境问题进入结构化人工处理路由
- [x] Reverse Sync：低置信或归属不确定时保留经过逐句安全过滤的模型建议
- [x] 补充真实噪音表达、危险建议和边界回归并完成本地综合验证
- [ ] 生产部署后使用测试专用号完成多种真实措辞的客户端收件验收

## 当前证据与边界

- 生产测试专用号实际发送“洗衣机不出水”；模型给出针对性建议且客人出站任务完成，但没有维修任务、员工通知或“已提交管家人工处理”。
- worker 总体健康，消息处理和客人发送任务均为 `COMPLETED`，等待数据库锁为 0；本次失败不是轮询、worker、积压或锁中断。
- `has_facility_fault_signal("洗衣机不出水")` 和 `is_service_request("洗衣机不出水")` 均为 `False`；同一本地门槛又在模型决定校验和会话设施路由中各阻断一次。
- 不增加设备词典、安全档位、第二个分类器或第二次模型调用；结构化设施归属只创建待人工确认任务，不授权退款、价格、预订或其他敏感操作。
- 本地继续负责紧急事件、明确私人或外部设施排除、危险建议过滤、任务成功真实性和模型不可用时的确定性兜底。

## 本地 Review

- 红测为 `17 failed, 4 passed`，分别证明提示词未覆盖状态/请求表达、模型设施字段被本地词表清除、会话路由再次阻断、低置信和不确定归属未降级建任务、维修请求仍发送中立安抚。
- 最小实现只修改现有模型决定校验、固定民宿提示词、设施回复路由和快速安抚匹配；没有增加设备词典、安全档位、分类器、模型调用、依赖或迁移。
- 设施及共享模型决定聚焦回归为 `255 passed`；冻结差异后的完整无外联回归为 `1298 passed, 15 skipped`，两项 warning 均为既有 Starlette/httpx 和 Alembic 配置弃用提示。
- Ruff 全仓通过，Mypy `117` 个源码文件无问题，`git diff --check` 通过；真实 DeepSeek、Hostex 和企业微信契约测试按门禁未启用，生产真实客户端收件保留为独立发布后验收。

## v1.3.12 生产发布 Review

- 提交和标签均指向 `ddaca58f5268a0b167fb1e21b78a1b7281e36b77`；main CI `33471354319` 与标签 CI `33471354382` 均成功。
- 生产备份位于 `/opt/yumi-backups/v1.3.12-20260901T045417Z`；源码 bundle、PostgreSQL custom dump 和 SHA-256 清单均已验证。
- 生产源码快进到目标提交，三个既存未跟踪环境备份保持不变；依赖安装层命中缓存，完整镜像构建约 5 秒，只替换 API。
- API 包和本机、公网 OpenAPI 均为 `1.3.12`，Alembic 为 `0023_approval_pii_final (head)`；API 重启次数为 0，部署后日志未发现 `ERROR`、`Traceback` 或 `Exception`。
- PostgreSQL 容器保持 `af11bb4a411f` / `2026-08-10T16:43:53.783849747Z`，重启次数为 0；公网健康为 `ok`，登录页返回 200，未认证诊断端点返回 401。
- 真实微信客户端收件仍待测试专用号发送实际原文后独立验收，不能用企业微信接口受理状态替代。

## v1.3.12 真实消息验收补充

- 测试专用号 customer `62` 的标签仍为“测试专用号”；真实原文依次为“洗衣机不出水”“马桶堵了”“晚上有噪音啊怎么办”“觉都睡不好”。
- 四条消息均创建 `MAINTENANCE / PENDING_CONFIRMATION` 任务；每条对应的 `wecom_send_internal_text` 和 `wecom_send_text` 均按顺序完成，尝试次数为 1、错误码为空。
- worker 综合健康返回 `ok`；564 个任务已完成，40 个待处理任务均未到执行时间，到期积压为 0，等待锁会话和未授予锁均为 0。
- “洗衣机不出水”“马桶堵了”和“觉都睡不好”的建议与场景匹配；“晚上有噪音啊怎么办”错误降级为“停止使用该设施”，因此多措辞内容验收未通过，不能将本轮标记完成。
- 根因位于 `ConversationService._facility_reply_text()`：整体置信度低或设施归属不确定时会丢弃模型正文；后续 `prepare_facility_issue_reply()` 已有逐句危险动作过滤，继续整体丢弃属于重复且过宽的安全门。
- 最小修正是不新增噪音关键词或安全档位：低置信或归属不确定时仍把模型正文交给既有逐句安全过滤；危险句被清除后继续使用统一降级文案，安全句则保留。

## 暂缓后的新消息检查

- 用户要求暂缓上一项通用降级修正，本次仅检查新消息和运行链路，没有修改业务代码。
- 新原文“外面很吵怎么办，觉都睡不好”得到关门窗、拉窗帘建议，但又提出“我可以帮您看看是否有耳塞提供”；没有创建人工任务或员工通知。
- 后续原文“可以”被当作普通对话继续回复，仍未创建人工任务；两条客人出站均为企业微信受理状态，用户已确认微信客户端实际收到，端到端投递验收通过。
- 新增 `wecom_process_message` 与 `wecom_send_text` 任务均为 `COMPLETED`、尝试次数 1、错误码为空；没有对应 `wecom_send_internal_text`。
- worker 综合健康为 `ok`，当前到期积压为 0，等待锁会话和未授予锁均为 0，近期日志未发现异常；失败属于语义路由，不是运行链路中断。
- 与上一项不同，本次共同根因是固定提示词只把“设施无法正常使用、异常现象或维修请求”纳入 `facility_issue`；住宿环境干扰未进入人工任务路径，因此普通回复清理也没有执行设施路径的“不追问、提交人工”约束。
- 本次仅确认投递链路成功；因回复仍含追问且没有提交人工任务，内容与业务动作验收保持未通过。

## 住宿问题语义泛化修复计划

- [x] 三段 Spec 已确认，用户明确回复“开始”
- [x] 固定提示词按“影响当前住宿且需要民宿处理”识别设施或环境问题
- [x] 结构化归属直接进入待确认任务，不增加关键词表或第二次模型调用
- [x] 安全模型建议按句过滤，过滤为空才使用统一降级文案
- [x] 完成聚焦测试、事务回归、Ruff、Mypy、差异检查和无外联全量测试

### 实施边界

- 复用现有 `facility_issue`、`MAINTENANCE / PENDING_CONFIRMATION` 和设施回复安全策略，不新增数据库字段、任务类型、分类器、依赖或模型调用。
- 本轮只编码和本地验证，不调用真实 DeepSeek、Hostex 或企业微信，不提交、推送、升级版本或部署生产。

### 本地 Review

- 两条最小红测修改前均失败：固定提示词缺少住宿环境语义，低置信安全噪音建议被整体替换为通用停用文案。
- 最小实现只扩展现有 `facility_issue` 提示词和说明、移除整体置信度丢弃，并把任务描述泛化为“住宿设施或环境问题”；没有增加词表、档位、结构、依赖或调用。
- 三条生产真实措辞纳入参数化回归；低置信安全建议保留，危险建议和追问继续由现有逐句策略清除，私人或外部问题边界保持不变。
- 聚焦模型决定、会话服务、回复安全策略和事务消息流测试为 `235 passed`；最终无外联全量测试为 `1301 passed, 15 skipped`。
- Ruff 全仓通过，Mypy `117` 个源码文件无问题，`git diff --check` 通过；两项 warning 均为既有 Starlette/httpx 和 Alembic 配置弃用提示。
- 真实 DeepSeek、Hostex 和企业微信契约测试按门禁跳过；未提交、推送、升级版本或部署，真实生产语义和客户端收件仍需后续独立授权验收。

## 发布 v1.3.13

- [x] 用户明确授权发布
- [x] 将版本升级为 `1.3.13` 并同步正式更新日志
- [x] 完成最新 Ponytail 发布审查
- [x] 提交、标记 `v1.3.13` 并首次推送 GitHub
- [x] 等待 main 与标签 CI 成功
- [x] 创建并验证生产备份
- [x] 仅重建并替换 API，保持 PostgreSQL 运行
- [x] 验证服务器源码、迁移、容器版本、健康、日志和公网入口
- [x] 使用测试专用号完成真实微信客户端收件验收

### v1.3.13 生产发布 Review

- 提交和标签均指向 `88efca988f2a87b232041a2a1c0f20e78165f3ec`；main CI `33527472202` 与标签 CI `33527472552` 均成功。
- 生产备份位于 `/opt/yumi-backups/v1.3.13-20260901T154631Z`；源码 bundle、PostgreSQL custom dump 和 SHA-256 清单均已验证，`.env` 与数据库归档权限为 `600`。
- 经 SHA-256 一致且验证完整的发布 bundle 将服务器源码快进到 v1.3.13；三个既存未跟踪环境备份保持不变。
- Docker 依赖层命中缓存，镜像构建约 5 秒；仅替换 API，PostgreSQL 容器保持 `af11bb4a411f` / `2026-08-10T16:43:53.783849747Z`，重启次数为 0。
- API 包和本机、公网 OpenAPI 均为 `1.3.13`，Alembic 为 `0023_approval_pii_final (head)`；本机与公网健康均为 `ok`，API 重启次数为 0，部署后日志未发现 `ERROR`、`Traceback` 或 `Exception`。
- 公网登录页返回 200，未认证诊断端点返回 401；本次没有前端改动，因此不重复浏览器视觉验收。
- 部署完成时尚未主动发送真实消息；企业微信语义、人工任务和客户端实际收件随后由测试专用号独立验收。

### v1.3.13 真实消息验收

- 测试专用号实际原文为“外面有人吵架  吵到我了”；系统创建任务 `478`，类型为 `MAINTENANCE / PENDING_CONFIRMATION`，描述为“客人反馈民宿设施或环境问题，待人工处理”。
- 消息处理、员工通知和客人发送任务均为 `COMPLETED`、尝试次数 1、错误码为空；机器人正文没有追问并包含“已提交管家人工处理”。
- 用户已确认微信客户端实际收到该回复，企业微信端到端收件验收通过。
- worker 综合健康为 `ok`；45 个待处理任务均未到执行时间，到期积压为 0，等待锁会话和未授予锁均为 0。

# 当前任务：风险优先运营台视觉与信息架构改造

- [x] 完成 Impeccable 与 Taste 综合审查并确认现状证据
- [x] 用户确认统一改造边界并明确回复“开始”
- [x] 调整总览、房态与任务中心的风险优先信息层级
- [x] 统一房态事实、运营准备、可信度和下一步的视觉语法
- [x] 修正房态日期范围文案、移动端密度和结构化语义
- [x] 完成服务排序、模板与响应式样式的聚焦回归
- [x] 执行桌面与移动端批量截图检查并完成一次修正
- [x] 冻结差异后运行最终最小充分门禁和差异自审

## 实施边界

- 保留现有 Jinja、原生 CSS/JavaScript、导航名称、URL、表单字段和业务状态，不新增前端框架、组件库、图表库、数据库迁移或运行依赖。
- 总览优先显示真实风险和今日周转；房间按同步可信度、今日周转、逾期任务和运营准备排序；稳定房间降为次级信息。
- 任务中心保留全部筛选能力，但快速队列最多四项，高级筛选使用原生渐进披露，移动端无需滚过完整筛选墙才能看到任务结果。
- 本轮只编码和本地验证，不提交、推送、升级版本、部署生产或调用真实 DeepSeek、Hostex、企业微信。

### 本地 Review

- 风险排序落在 `admin_operations_service._room_risk_sort_key`：依次按同步可信度、今日周转（`_OCCUPANCY_TURNOVER_RANK`）、逾期任务、运营准备（`_READINESS_RANK`）、开放任务，最后用房源标题和主键做确定性 tie-break；房间与时间轴在 `_room_items` 内成对排序，模板改用 `OperationsSnapshot.timeline_for()`，不再依赖两个序列的下标对齐。
- 稳定房间由 `RoomOperationItem.needs_attention` 判定（房态不可信、有逾期、今日进出、或运营准备为维修/保洁/待检查），经 `attention_rooms` / `stable_rooms` 分组；房态不可信时所有房间都留在关注组，不会被降级。
- 总览新增 `AdminDashboardService._count_overdue_tasks`，逾期口径与 `list_open_task_counts` 一致（服务日期早于本地当日且任务未终态）；页面拆成「需要人工处理的风险」和「今日周转与工作量」两个 `aria-labelledby` 分区，启用房间数降级进房态面板，指标为零时不染危险色。
- 时间轴范围文案改为由 `timeline_start_date` / `timeline_end_date` / `timeline_past_days` 如实描述；原页面只写「未来 N 天」，实际渲染的是前 2 天加今天加未来 N 天。房态范围页签本身保持不变。
- 视觉语法统一到 `components/ui.html` 的 `occupancy_badge` 和 `readiness_badge`：可信度不再混进入住状态取色，琥珀色只表示今日周转，运营准备除维修（危险）和可入住（成功）外一律中性，有逾期时「下一步」转危险色。房间标题改为 `h3`，时间轴改为 `ol`。
- 任务中心快速队列收敛到四项，已失效改由高级筛选进入；完整筛选包进 `details.filter-disclosure`，默认收起、有生效筛选时自动展开，移动端不必滚过整面筛选墙才能看到结果。
- 移动端密度：指标卡在 ≤520px 改为单列紧凑行（标签在左、数值在右），房态时间轴降低格高保持三列均衡。
- 新增测试 7 条（房间风险排序与分组、房态不可信不降级、时间轴范围事实、总览逾期指标、总览风险分区顺序、稳定房间折叠与范围文案、任务中心快速队列与渐进披露）。修改既有测试 2 条：一致读顺序断言补入 `overdue`；任务筛选断言由「已失效快速队列链接」改为「高级筛选中的已失效选项」，筛选能力本身未减少。
- 冻结差异后的最终门禁：全量 `1309 passed, 15 skipped`（跳过项全部是需显式开启的真实 DeepSeek、百居易、企业微信契约测试），Ruff 全仓通过，mypy `117` 个源文件通过，`pip check` 无破损依赖，`compileall` 通过，`git diff --check` 通过。两项 warning 均为既存 Starlette/httpx 弃用提示。
- 截图检查：桌面 1440×900 与移动 390×844 各三张（总览、房态、任务中心），基于真实模板与真实 CSS/JS 渲染。一轮修正四处后复检通过：房态两类徽标撞色、移动端时间轴残行、任务中心移动卡片逾期标记不显眼、待关注按钮词中折行。
- 本轮只做本地编码与验证：没有提交、推送、升级版本或部署，没有调用真实 DeepSeek、百居易和企业微信，受保护的未跟踪项目总结保持未读、未改、未暂存。
- 遗留待人工清理（挂载目录禁止删除）：`.stage/`、`run-gate.command`、`.rm-probe`，以及 `.git/index.lock.stale*` 两个改名后的残留锁。这些都不属于本次改造的交付内容。

## 发布 v1.3.14

- [x] 用户明确授权发布
- [x] 将版本升级为 `1.3.14` 并同步正式更新日志
- [x] 完成发布前差异审查（本会话未安装 ponytail 技能，由本会话自行完成等价审查，记录见下）
- [x] 提交、标记 `v1.3.14` 并推送 GitHub
- [x] 等待 main 与标签 CI 成功
- [x] 创建并验证生产备份
- [x] 仅重建并替换 API，保持 PostgreSQL 运行
- [x] 验证服务器源码、容器版本、健康、日志和公网入口
- [x] 登录后台完成桌面与移动端页面视觉验收（本版本有前端改动，不能省略）

### 发布前置事实

- 本版本没有数据库迁移，Alembic head 仍为 `0023_approval_pii_final`；没有依赖、Dockerfile、Compose 或运行配置变化，因此部署只需替换 API 容器。
- 冻结差异后的本地门禁：全量 `1309 passed, 15 skipped`，Ruff 通过，mypy `117` 个源文件通过，`pip check` 无破损依赖，`compileall` 与 `git diff --check` 通过。
- 版本号升级和更新日志之后未改动业务源码，仍需在提交前复跑一次版本暴露回归确认包元数据为 `1.3.14`。
- 本会话运行在云端容器，通过桥接访问本机仓库：没有 `/ponytail-review`、没有出网、也没有生产服务器访问，因此提交、推送、备份和部署必须在具备 Ponytail 门禁与网络的环境执行。

### 发布前差异审查记录

- 审查范围为 14 个文件的完整工作区差异。本会话未安装 ponytail 技能（技能检索为空，仅有 `rin-skills` 插件），因此这是本会话自行完成的等价审查，不能当作 `/ponytail-review` 的输出。
- 修正一：`AdminDashboardService._count_overdue_tasks` 不按房源是否启用过滤，与相邻的「待办任务」同源，逾期数始终是待办数的子集；原注释和更新日志误称与房态页口径一致，房态页的 `list_open_task_counts` 会 join `PropertyProfile.is_active`，存在停用房源遗留任务时两处不同。查询保留，注释和更新日志改为如实说明。
- 修正二：`RoomOperationItem.needs_attention` 原本用 `_READINESS_RANK[self.status] <= 1` 判定，把房间分组耦合到排序权重表，调整权重会静默改变分组。改为显式的 `_ATTENTION_READINESS` 状态集合，行为等价。
- 修正三：房态页无待处理房间时的空态文案原为「启用房源都已就绪」，但稳定房间也包含未开始和已入住，改为「没有房间需要今天介入，可在下方展开稳定房间查看」。
- 修正后的聚焦验证集为 `30 passed`（运营服务分组与排序、版本源、后台路由模板渲染），Ruff 全仓通过，mypy `117` 个源文件通过；`git diff --check` 首次报出 `tasks/todo.md` 末尾空行并已修复，复跑通过。
- 未重跑全量：业务逻辑自 `1309 passed, 15 skipped` 的冻结门禁之后只替换了一个等价判定，其余为注释、文案和版本号。

### v1.3.14 生产发布 Review

- 提交与注释标签均指向 `ca4dc1e074fa2090b9b8b684619beb6ed4b3d727`；`git ls-remote` 独立确认 GitHub 上 `refs/heads/main` 与 `refs/tags/v1.3.14`（标签对象 `6dde391`）都解析到该提交。main CI `#17` 与标签 CI `#18` 均成功。
- 生产备份位于 `/opt/yumi-backups/v1.3.14-20260902T154431Z`：源码 bundle 通过 `git bundle verify` 且记录完整历史；`.env` 与 PostgreSQL dump 权限均为 `600`；dump 在容器内 `pg_restore -l` 读出 360 行目录清单；私有上传目录已归档；`MANIFEST.txt` 与 `SHA256SUMS` 齐全；旧镜像 `d4496b87` 标记为 `yumi-homestay-bot-api:rollback-v1.3.13`。
- 发布包 SHA-256 `56fa36dc…` 在服务器端比对一致后才 `git bundle verify` 并快进；服务器由 `88efca9` 快进到 `ca4dc1e` 并断言 HEAD 相符，三个既存未跟踪环境备份保持不变。
- 只重建 API：新容器 `fb070be1`、新镜像 `cc7adbac`，运行用户 `uid=10001(app) gid=10001(app)`，重启次数 0。PostgreSQL 未重建也未重启，仍为 `af11bb4a411f`，启动时间保持 `2026-08-10T16:43:53.783849747Z`，重启次数 0。
- 容器包版本为 `1.3.14`，`alembic current` 仍为 `0023_approval_pii_final (head)`，确认启动时的 `alembic upgrade head` 是空操作、未变更结构。
- 本机与公网 `/health` 均为 `ok`，公网 OpenAPI 为 `1.3.14`；公网 `/employee/login` 返回 200 且带 `no-store`、`frame-ancestors 'none'`、`DENY`、`no-referrer`、`nosniff`；未登录访问 `/employee/approvals` 返回 401。私有上传仍挂载 `/opt/yumi-data/private_uploads -> /app/data/private_uploads`，文件数为 0。部署后 10 分钟内 `ERROR`、`Traceback`、`Exception` 计数为 0。
- 登录后视觉验收在真实生产页面完成，桌面与 375px 窄屏各一遍：总览两组分区顺序正确，风险指标为零时不染危险色（待审批 0 为常规色，需人工关注 366 与逾期任务 86 为危险色），侧栏版本显示 `v1.3.14`；房态页时间轴文案为「时间轴覆盖 9月1日 至 9月6日，含已过去的 2 天与今天」，与实际渲染的六格一致。
- 真实数据验证了风险排序：今日周转房间排首位，其后是三间今日退房房间按逾期数 18、17、11 降序，再是三间近期空置房间按逾期数 11、8、4 降序，与 `_room_risk_sort_key` 的设计完全一致。本次七间房全部需要今天处理，因此稳定房间折叠区按预期不出现。
- 四处截图修正在生产确认生效：运营准备徽标为中性色、不再与今日周转的琥珀色撞色；窄屏时间轴为均衡三列；任务中心移动卡片的「已逾期」为危险色徽标；「N 项待关注」按钮不再从词中间折行。任务中心快速队列为四项，高级筛选默认收起、带 `overdue=true` 时显示「已生效」并自动展开。
- 本次发布没有主动调用 DeepSeek、百居易或企业微信，没有发送真实消息；受保护的未跟踪项目总结保持未读、未改、未暂存。
