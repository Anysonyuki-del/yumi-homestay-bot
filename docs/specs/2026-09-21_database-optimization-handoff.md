...
### 15.4 1.38.0 已部署（语义检索保持关闭）

| 项 | 结果 |
| --- | --- |
| 发布 | `main` = `5621e94`，标签 `v1.38.0`；CI 在 SQLite 与隔离 PostgreSQL 上都执行了迁移 `0027`，1737 通过，PG 回归 18 通过 |
| 部署前备份 | `pre-v1.38.0-20260922T030107Z`，可读性复验通过 |
| 迁移 | 生产 `alembic_version = 0027_knowledge_embeddings`，`knowledge_embeddings` 为空表 |
| 配置兼容 | 在生产容器里只读解密两个已存配置版本（含当前激活版本）：都能读出且通过校验，`embedding_enabled=False` |
| 运行 | 包版本 1.38.0，重启 0 次，启动后没有告警或调用栈；数据库容器未动；公网健康有应答 |

下一步都需要 key：把 key 写进 `~/.config/yumi/siliconflow.env`（`chmod 600`）后，按 §15.3 调参、冻结、跑第二套留出集；达标后再在「接口设置」里开启。
